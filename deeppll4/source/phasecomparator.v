...
module phasecomparator(InputSignal, OutputSignal, MainClock, Rst_n, Lead, Lag, InputSignalEdge, OutputSignalEdge, Lock, SynchronousSignal, PeriodCount);
    ...
    /*	计算输入信号的频率（时钟周期数）	*/
    reg flag;
    reg [15:0] cnt, PeriodCount;
    always @(posedge MainClock or negedge Rst_n)
        begin
            if(!Rst_n)
                cnt <= 0;
            else begin
    ...
    always @(posedge InputSignalDownEdge or negedge Rst_n)
        begin
            if(!Rst_n) begin
                flag = 1'b1;
            end
